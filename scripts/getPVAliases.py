...
def request_file_dest(prompt: str,
    ...
    default: str
        ...
        Default is '{current_dir}/tempt.txt'
    Returns
    -------
    str
...
def flatten_list(input_list: list):
    """
    Flatten a list of lists and find its unique values.
    Note: loses order due to set() call.
    """
    _result = [e for l in input_list for e in l]
    return list(set(_result))


def search_file(file:str,output:list=None,patt:str=None,prefix:str='',
                color_wrap:Fore=None):
    """
...
def fix_json(raw_data: str):
    """
    Fixes JSON format of find_ioc output.
    """
...
def find_ioc(hutch: str=None, patt: str=None):
    ...
    -------
    str
        Returns the output as a list of dict after json formatting

    """
    hutch_list = ['xpp', 'xcs', 'cxi', 'mfx', 'mec', 'tmo', 'rix', 'xrt',
		 'aux', 'det', 'fee', 'hpl', 'icl', 'las', 'lfe', 'kfe', 'tst',
		 'thz', 'all']
    # check hutches
...
def find_parent_ioc(file: str=None, path: str=None):
    """
...
def build_table(input_data: list,columns: list=None,**kwargs):
    """
...
def acquire_aliases(dir_path:str,ioc:str):
    """
    Scans the st.cmd of the child IOC for the main PV aliases.
    Returns a list of dicts for the associations.
    """
    _d = fix_dir(dir_path)
    ...
    output = [re.sub(r'\"|\)|RECORD\=|ALIAS\=','',s).split(',') for s in _temp]
    return [{'record':s[0],'alias':s[-1]} for s in output]


def process_alias_template(parent_release: str,record: str, alias: str):
    """
    Opens the parent db/alias.db file and preps it for processing.
    Returns a list of the substituted associations
    """
    ...
    for _ioc in data:
        if _ioc.get('disable') is not True:
            ...
            for i,a in enumerate(alias_dicts):
                ...
                _output = [f"{l[0]:<61}{l[-1]:<61}" for l in alias_list]
                ...
                if (i==0)|((show_pvs is True)&(skip_all is False)):
                    ...
                    _temp = [{'PV' : l[0],'Alias' : l[-1]} for l in alias_list]
                    print(Fore.LIGHTGREEN_EX
